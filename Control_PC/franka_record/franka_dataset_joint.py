...
from generate.parquet_writer_joint import ParquetWriter
from generate.video_writer import VideoWriterModule
from generate.stats_generator_joint import StatsGenerator
from generate.meta_writer_joint import MetaWriter
...
class FrankaDataset:
    ...
    def add_episode(
        self,
        episode_index: int,
        joint_state: np.ndarray, # shape (frame_count, 7)
        gripper_widths: np.ndarray,
        view1_frames: np.ndarray,
        view2_frames: np.ndarray,
        view3_frames: np.ndarray,  # For third camera view
    ) -> None:
        ...
        vid_dir_view1 = (
            self.root / "videos" / chunk_name / "observation.images.image_front_eye"
        )
        vid_dir_view2 = (
            self.root / "videos" / chunk_name / "observation.images.image_rear_eye"
        )
        vid_dir_view3 = (
            self.root / "videos" / chunk_name / "observation.images.image_front_view"
        )
        vid_dir_view1.mkdir(parents=True, exist_ok=True)
        vid_dir_view2.mkdir(parents=True, exist_ok=True)
        vid_dir_view3.mkdir(parents=True, exist_ok=True)
        ...
        # Write Videos (view1 & view2 & view3)
        ...
        video_path_view3 = vid_dir_view3 / f"episode_{episode_index:06d}.mp4"
        ...
        self.video_writer.write_episode(view3_frames, video_path_view3)  # Third view
