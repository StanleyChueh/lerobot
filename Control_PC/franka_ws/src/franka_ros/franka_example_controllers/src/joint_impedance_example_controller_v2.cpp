...
// // ------------------- 改良版 Cubic Spline -------------------
// std::vector<std::vector<double>> cubicSplineInterpolation(
//     const std::vector<double>& q_start,
//     const std::vector<double>& q_goal,
//     const std::array<double, 7>& qdot_start,          // 當下速度
//     size_t max_steps = 100,         // 最大步數
//     double max_joint_velocity = 0.5 // rad/s，每步最大位移限制
// ) {
...
//         delta[j] = q_goal[j] - q_start[j];
//         if (std::fabs(delta[j]) > max_delta) {
//             max_delta = std::fabs(delta[j]);
//         }
//     }

//     size_t steps = max_steps;
//     if (max_delta > 0) {
//         steps = std::max(max_steps, static_cast<size_t>(std::ceil(max_delta / max_joint_velocity)));
//     }

//     std::vector<double> v_start(dof), v_goal(dof);
//     for (size_t j = 0; j < dof; ++j) {
//         v_start[j] = qdot_start[j];
//         v_goal[j] = 0.3 * delta[j];
//     }

//     for (size_t i = 0; i < steps; ++i) {
//         double t = static_cast<double>(i) / (steps - 1);
//         std::vector<double> q_step(dof);
//         for (size_t j = 0; j < dof; ++j) {
//             double a0 = q_start[j];
//             double a1 = v_start[j];
//             double a2 = 3 * delta[j] - 2 * v_start[j] - v_goal[j];
//             double a3 = -2 * delta[j] + v_start[j] + v_goal[j];
//             q_step[j] = a0 + a1 * t + a2 * t * t + a3 * t * t * t;
//         }
...
void JointImpedanceExampleController::jointTrajectoryCallback(
  ...
  // spline_index_ = 0;
  traj_path_ = cubicSplineQdQdd(q_curr, positions, robot_state.dq, 100, 0.5);
  traj_index_ = 0;
...
void JointImpedanceExampleController::starting(const ros::Time& /*time*/) {
  ...
  tau_log_file_.open("/home/csl/franka_ws/logs/franka_tau_log.csv");
  tau_log_file_ << "step";
...
void JointImpedanceExampleController::update(const ros::Time& /*time*/,
  ...
  if (!spline_path_.empty() && spline_index_ < spline_path_.size()) {
    for (size_t i = 0; i < 7; ++i) {
      target_positions_from_topic_[i] = spline_path_[spline_index_][i];
    }
    spline_index_++;
    if(spline_index_ == spline_path_.size()){
      std::cout << "spline finish"<<std::endl;
    }
  }
  ...
  if (elapsed.toSec() >= 120.0) {
    ROS_INFO("2 minutes elapsed. Stopping controller and saving log...");
