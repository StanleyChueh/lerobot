...
def format_attention(attn, observation):
    attn_dict = {}
    ...
    # 假設最後一層 attention
    last_attn = attn[-1]  # (B, heads, tokens, tokens)

    # average heads
    attn_map = last_attn.mean(dim=1)[0]  # (tokens, tokens)

    # CLS → image tokens
    spatial_attn = attn_map[0, 1:]


    attn_dict["global"] = spatial_attn.detach().cpu().numpy()
...
def sanity_check_dataset_robot_compatibility(
    ...
    with (
        ...
        # 🔥 改這裡
        if return_attention:
            action, attn = policy.select_action(
                observation,
    ...
    if return_attention:
        attn_dict = format_attention(attn, observation)
        return action, attn_dict
