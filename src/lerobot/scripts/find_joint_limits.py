...
def find_joint_and_ee_bounds(cfg: FindJointLimitsConfig):
    ...
    # URDF optional
    has_urdf = hasattr(cfg.robot, "urdf_path") and getattr(cfg.robot, "urdf_path", None)
    kinematics = RobotKinematics(cfg.robot.urdf_path, cfg.robot.target_frame_name) if has_urdf else None
    if not has_urdf:
        print("[WARN] No URDF on this robot config; skipping end-effector bounds.")
