...
def get_top_tokens_from_hidden_state(hidden_state, lm_head, tokenizer, top_k=5):
    """
    將隱藏狀態投影到詞表空間，並回傳機率最高的前 k 個 tokens
    """
    # hidden_state shape: (1, seq_len, hidden_dim)
    # 我們通常取最後一個 token 的 hidden state (代表當前預測趨勢)
    last_token_feat = hidden_state[:, -1, :] 
    
    # 投影到詞表 (Unembedding)
    logits = lm_head(last_token_feat) # (1, vocab_size)
    ...
    for i in range(top_k):
        ...
        tokens.append(f"{token_text.strip()} ({prob_val:.2f})")
