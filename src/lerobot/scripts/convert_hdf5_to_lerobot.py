...
def _scan_episodes_libero(hdf5_files: list) -> list:
    """Scan our LIBERO height-demo HDF5 files (root-level ep_NNN keys)."""
    infos = []
    for path in hdf5_files:
        _print(f"Scanning {path} ...")
        with h5py.File(str(path), "r") as f:
            for name in sorted(k for k in f.keys() if k.startswith("ep_")):
                infos.append((str(path), name))
    return infos
...
def hdf5_to_lerobot(
    ...
    _scan_fn = _scan_episodes_libero if fmt == "libero" else _scan_episodes
    _load_fn = _load_episode_libero  if fmt == "libero" else _load_episode

    # ── Phase 1: scan episode names (no frame loading) ────────────────────────
    ep_infos = _scan_fn(hdf5_files)
    if not ep_infos:
    ...
    first = _load_fn(ep_infos[0][0], ep_infos[0][1], cameras)
    ...
    def _loader_worker():
        for ep_path, ep_name in ep_infos:
            load_q.put(_load_fn(ep_path, ep_name, cameras))
    ...
    features: dict = {
        "action":            {"dtype": "float32", "shape": [action_dim],
                              "names": _joint_names(action_dim, is_action=True,  fmt=fmt)},
        "observation.state": {"dtype": "float32", "shape": [state_dim],
                              "names": _joint_names(state_dim, is_action=False, fmt=fmt)},
    ...
    info = {
        "codebase_version": "v3.0", "robot_type": "franka" if fmt == "libero" else "openarm",
    ...
    robot_id = "franka" if fmt == "libero" else "openarm"
    _print(f"  ds = LeRobotDataset(repo_id='local/{robot_id}', root='{out_dir}', download_videos=False)")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--format",      default="isaaclab",
                    choices=["isaaclab", "libero"],
                    help="HDF5 source format: isaaclab (OpenArm) or libero (Franka height demos)")
    args = ap.parse_args()
