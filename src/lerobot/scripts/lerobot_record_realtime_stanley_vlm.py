    ...
    while timestamp < control_time_s:
        ...
        if policy is not None and preprocessor is not None and postprocessor is not None:
            ...
            if display_data and hasattr(model, "attn_records"):
                ...
                # Change this to "expert_cross" or "vlm_self" to switch views!
                target_attn = "vlm_self"

                layer_ids = [k[0] for k in model.attn_records.keys() if k[1] == target_attn]
                print(f"{target_attn} layer count after predict:", len(layer_ids))
                ...
                if len(layer_ids) > 0:
                    ...
                    # Use the toggle variable to pull the correct list
                    attn_list = model.attn_records.get((final_layer, target_attn), [])
                    print(f"final layer {target_attn} records:", len(attn_list))
