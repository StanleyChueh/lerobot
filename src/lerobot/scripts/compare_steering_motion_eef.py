...
python src/lerobot/scripts/compare_steering_motion_eef.py \
  ...
  --demo-n-episodes 20 \
  --prompt-repo-ids \
    ethanCSL/eval_steering_low_prompt \
    ethanCSL/eval_steering_high_prompt \
  --prompt-labels prompt_low prompt_high \
  --prompt-n-episodes 20 \
  --plot-start-time 0 \
  --plot-end-time 10 \
...
def build_rows_from_episodes(eps, n_episodes=None):
    selected_eps = select_episode_range(
        eps,
        start_idx=None,
        end_idx=None,
    ...
    return [
        compute_episode_metrics(ep_idx, ep)
        for ep_idx, ep in sorted(selected_eps.items())
    ]


def add_dataset_to_results(
    all_rows,
    all_summaries,
    out_dir,
    label,
    ...
    eps = load_episodes(repo_id, root=root, max_frames=max_frames)
    rows = build_rows_from_episodes(eps, n_episodes=n_episodes)
...
def main():
    ...
    parser.add_argument(
        ...
        help=(
            "Two prompt-only steering eval repos to include in the analysis. "
            "Default order is low-prompt then high-prompt."
        ),
    )
    parser.add_argument(
        "--prompt-labels",
        type=str,
        nargs=2,
        default=["prompt_low", "prompt_high"],
        help="Labels for --prompt-repo-ids. Default order matches low-prompt then high-prompt.",
    )
    parser.add_argument(
        "--prompt-n-episodes",
        type=int,
        default=20,
        help="Number of episodes to use from each prompt-only steering repo.",
    )
    ...
    requested_labels = list(args.labels) + list(args.prompt_labels)
    if len(set(requested_labels)) != len(requested_labels):
        parser.error("--labels and --prompt-labels must not contain duplicate label names.")
    ...
    for label, repo_id in zip(args.prompt_labels, args.prompt_repo_ids):
        add_dataset_to_results(
            ...
            root=args.root,
            max_frames=args.max_frames,
            n_episodes=args.prompt_n_episodes,
        )
    ...
    if args.demo_repo_id is not None:
        print(f"  {out_dir / f'paper_low_high_height_20episodes_{interval_tag}.png'}")
        print(f"  {out_dir / f'paper_low_high_baseline_dataset_height_20episodes_{interval_tag}.png'}")
