...
policy.model.vlm_with_expert.debug_cross_attn = False
...
def extract_smolvlm_self_attention(policy):
    meta = policy.model.vlm_with_expert.last_attn
    attn = meta["prefix_self_attn"][0]   # [heads, Q, K]
    attn_matrix = attn.mean(0)           # [Q, K]

    num_img_tokens = meta["prefix_meta"]["num_image_tokens_per_cam"]

    # 對所有 image queries 平均
    attn_1d = attn_matrix.mean(0)
...
def process_heatmap(heat_1d, original_image_size=(480, 640), model_input_size=(512, 512)):
    ...
    """
    heat_1d = heat_1d.detach().float().cpu()
    ...
    heat_tensor = heat_2d.detach().clone().unsqueeze(0).unsqueeze(0).float()
    ...
    H, W = heat_512.shape[-2:]

    y1 = max(0, pad_h)
    y2 = min(H, pad_h + resized_h)
    x1 = max(0, pad_w)
    x2 = min(W, pad_w + resized_w)

    heat_valid = heat_512[0, 0, y1:y2, x1:x2]
...
def compute_deterministic_attention(policy, batch_pp, device):
    """
    batch_pp MUST be output of preprocessor
    """
    policy.eval()
    ...
    # 2. language (FROM PREPROCESSOR, NOT POLICY)
...
for global_idx in range(start_idx, end_idx):
    ...
    compute_deterministic_attention(policy, batch_pp, device)

    # --- ATTENTION EXTRACTION ---
    heat_front_1d, heat_top_1d = extract_aggregated_attention(policy, target_indices)

    with torch.no_grad():
        batch_green = dict(observation_batch)
        batch_red   = dict(observation_batch)

        batch_green["task"] = "Pick the green cube."
        batch_red["task"]   = "Pick the red cube."

        batch_green_pp = preprocessor(batch_green)
        batch_red_pp   = preprocessor(batch_red)

        for k, v in batch_green_pp.items():
            if isinstance(v, torch.Tensor):
                batch_green_pp[k] = v.to(device)
        for k, v in batch_red_pp.items():
            if isinstance(v, torch.Tensor):
                batch_red_pp[k] = v.to(device)

        chunk_green = policy.predict_action_chunk(batch_green_pp)  # (B, T, action_dim)
        chunk_red   = policy.predict_action_chunk(batch_red_pp)
        diff = torch.norm(chunk_green[:, 0] - chunk_red[:, 0]).item()  # 只比第一步
        print(f"[ACTION SENSITIVITY] ||green - red|| = {diff:.6f}")
