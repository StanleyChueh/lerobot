...
from franka_dataset_2cam import FrankaDataset
...
class DataRecorderNode(object):
    ...
    def try_record(self, episode_index):
        ...
        if self.pose is None or self.gripper is None:
            return False
    ...
    def convert_to_dataset_format(self, return_paths_only=True):
        ...
        if return_paths_only:
            return eef_position, eef_orientation, gripper_width, front_paths, agent_paths

        # else: load into memory (use with caution)
        view1_frames = []
        view2_frames = []
        import cv2
        for i, (fp, ap) in enumerate(zip(front_paths, agent_paths)):
            f = cv2.imread(fp)
            a = cv2.imread(ap)
            if f is None or a is None:
                rospy.logwarn(f"Missing image at index {i}: {fp}, {ap}")
                continue
            view1_frames.append(f)
            view2_frames.append(a)
...
def main() -> None:
    ...
    try:
        ...
        while episode_index < args.num_episodes and not recorder.exit_all:
            ...
            timeout = 5.0
            start = time.time()
            while (recorder.front_eye_image is None or recorder.agent_image is None) and time.time() - start < timeout:
            ...
            with tqdm(total=num_frames, desc=f"Ep {episode_index:03d} Frame", ncols=80) as pbar:
                while len(recorder.records) < num_frames:
                    ...
                    if recorder.front_eye_image is not None and recorder.agent_image is not None:
                    ...
                    elif recorder.front_eye_image is None or recorder.agent_image is None:
                        pass
            ...
            # 如果有錄到數據 (正常完成或按右鍵)
                ...
                eef_position, eef_orientation, gripper_width, front_paths, agent_paths = recorder.convert_to_dataset_format(return_paths_only=True)

                if front_paths is not None and len(front_paths) > 0:
                    try:
                        dataset.add_episode(episode_index, eef_position, eef_orientation, gripper_width, front_paths, agent_paths)
